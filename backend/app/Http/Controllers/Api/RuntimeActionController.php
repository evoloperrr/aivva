...
class RuntimeActionController extends Controller
{
    ...
    public function events(Request $r,Aivva $a):StreamedResponse{$this->authorizeRuntime($r,$a);$lastVersion=max(0,(int)$r->query('afterVersion',0));$lastActionId=(string)$r->query('afterActionId','');return response()->stream(function()use($a,$lastVersion,$lastActionId){$deadline=microtime(true)+(int)config('aivva.runtime.event_stream_seconds',25);$seenVersion=$lastVersion;$seenId=$lastActionId;echo "retry: 1500\n\n";while(microtime(true)<$deadline&&!connection_aborted()){$action=AivvaRuntimeAction::query()->where('aivva_id',$a->id)->latest('updated_at')->first();if($action&&($action->id!==$seenId||(int)$action->version>$seenVersion)){$seenId=$action->id;$seenVersion=(int)$action->version;$event=match($action->status){RuntimeActionStatus::Requested=>'AIVVA_ACTION_REQUESTED',RuntimeActionStatus::Cancelled=>'AIVVA_ACTION_CANCELLED',RuntimeActionStatus::Expired=>'AIVVA_ACTION_EXPIRED',default=>'AIVVA_ACTION_UPDATED'};echo 'event: '.$event."\n".'data: '.json_encode($this->serialize($action),JSON_UNESCAPED_SLASHES)."\n\n";}else{echo ": heartbeat\n\n";}if(ob_get_level()>0)@ob_flush();flush();usleep(750000);}},200,['Content-Type'=>'text/event-stream','Cache-Control'=>'no-cache, no-store, private','X-Accel-Buffering'=>'no']);}
